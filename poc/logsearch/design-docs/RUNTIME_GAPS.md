...
### 1. 文字列リテラルに逃げ道が無い ★★★ — この POC 最大の実害

**無いもの**: `\"` (lexer が拒否し `\u{22}` を案内する)、raw リテラル
`r"..."`、補間を止める手段。`{` は**どの文字列でも必ず補間**として読まれる。

**困ること**: JSON を喋るサーバなので直撃する。現物の数:

| 回避の形 | 箇所 |
|---|---|
| `\u{22}` (二重引用符 1 文字) | **435 出現 / 137 行** (15 ファイル) |
| `{{` / `}}` (ブレースの二重化) | **128** |

```rust
body.push_str("{{\u{22}error\u{22}:\u{22}")   # src/http.t:583
```

**回避策**: 上のとおり書ける。**動くが読めない** — HTTP とクエリの
応答組み立てが、この 2 つの逃げ方だけで埋まる。極端な現れ方が
`src/ui.t` で、**166 行中 133 行が `out.put_str("...")` 1 行ずつ**。
同ファイルの冒頭がその運用規約を書いている — 「`"` は書けないので
ページ全体をシングルクォートで通す」「ブレースは全部二重にする」。
**手で二重化を守る形なので、抜けは起動時まで落ちない** (テストではなく
本番の初回描画で補間エラーになる)。

**入ったら**: `r"..."` が 1 つ入れば `r"{"error":"` と書ける。435 + 128
箇所が消え、JSON の形がそのままソースに出る。→ todo の
**NEW-FEATURES「raw / multi-line string literal」** (lexer 拡張のみ)。

**ついでの食い違い**: `docs/language.md` は「複数行リテラルは未対応」と
書いているが、**生の改行を含む `"..."` は実際には通る**。文書か実装の
どちらかが古い。
