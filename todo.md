...
154. **JIT Phase 2 拡張** — Phase 1 / 2a (Cast) / 2b (print/println) / 2c (heap builtins) / 2c-2 (ptr_read/write) / 2g (sizeof) / 2f (generic) / 2d (struct field) / 2h (compile cache) は完了。残: (d-2) struct as param/return + method dispatch、(e) allocator stack 連携。サポート範囲のまとめは `JIT.md`
