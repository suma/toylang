...
- **GENERIC-ENUM-HOF-UNIFY** ★★ — generic enum に対する stdlib HOF (`Option::map` 等) を impl-block body で書くと、`match self { Some(v) => Some(f(v)), None => None }` の 2 arm が 同じ enum に対して異なる `Generic(?)` 置換で instantiate され (片方が `Struct(?, ..)`、片方が `Enum(?, ..)` として型チェッカに届く)、実行時の意味は正しいのに unifier が body を弾く。回避策は `match` を直に書くか、具体型に対する free function にすること。`interpreter/tests/closure_tests.rs` の Phase 7 コメントが参照している。
