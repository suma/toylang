...
- **クロージャ / ラムダ**: `fn(params) -> R { body }` 形式の anonymous function literal、関数型 `(T1, T2) -> R` で parameter / return 位置に書ける。bind は `val f = fn(x: i64) -> i64 { x + 1i64 }`。free var capture は creation time の snapshot (primitive は値コピー、compound は Rc 共有)。**backend coverage**: interpreter は full support (literals + captures + HOF args + return + nest)、JIT は silent fallback (closure を含む function は interpreter 経路に流れる)、AOT compiler は **`val name = fn(...) -> R { body }` 直接呼び出し (capturing も non-capturing も) + closures-as-arguments to HOFs (Phase 5b indirect call) を対応** — 残: capturing closure を HOF 引数として渡す / closure を return / field 格納 (Phase 6b 残、env+fn-ptr の thunk 表現が必要)。captures は 8-byte scalar (i64/u64/f64/bool) のみ。詳細は [`docs/language.md` → Closures](docs/language.md)。
