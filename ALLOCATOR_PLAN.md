...
- [ ] **scope-bound auto-cleanup — FixedBuffer** — 同形の `with allocator =
      FixedBuffer::new(cap) { ... }` でバッファを scope exit 時に release。
      現状 stdlib `FixedBuffer` には `drop` method が無く、runtime にも
      `__builtin_fixed_buffer_drop` builtin が無いので、まず runtime
      hook + builtin を追加してから AOT/interpreter の lower を Arena と
      同形にする。優先度低（FixedBuffer は capacity が compile-time
      で決まる用途が多く long-lived ではない）。
