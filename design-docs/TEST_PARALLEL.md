...
> **状態: P0〜P4 landing 済み (2026-09-11)。** 残るのは P5 (`serial`)、
> P6 (`--backend all` との合成)。
...
- **rayon を `toy` に足さない。** ジョブが粗くて work-stealing の細かさが
  ...
  と `AtomicUsize` で足りる。
- **長いジョブから先に (P4)。** ジョブ長の分散が大きい (`poc/logsearch` は
  1 本が数秒、他は ms) ので、順序が speedup をほぼ決める。前回の所要時間を
  `build/<profile>/.testtimes` に記録して降順に並べる。
  ファイルではなく `(file, line, name)` をキーにする — テストを 1 本足した
  だけで履歴が全部捨てられないように。**測っていないジョブは末尾ではなく
  先頭**に置く (設計時は末尾と書いたが、実装して考え直した): 未知が長い
  ジョブである可能性があり、**2 つの間違いは大きさが違う** — 短いジョブを
  早く始める損は無く、長いジョブを遅く始める損はその長さ全部である。
  同値は plan 順のままなので、履歴の無い初回は今までどおり動く。
  **レーンごとに別の記録**にする (同じブロックが AOT では ms、VM では
  秒なので、混ぜると推定がどちらの数字か分からなくなる)。
...
**分割するかは履歴が決める (P4)。** 1 ファイルを per-test に割ると、
そのファイルからテストを取ったワーカーの数だけ front end を払い直す
(検査済みプログラムは `Rc` を持つので共有できない)。**割って得なのは
「そのファイルのテストの合計 > そのファイルの front end」のときだけ**で、
どちらも前の run が測っている。ミリ秒のテストが 9 ファイルに散っている
スイートは、割ると検査のやり直しに全部持っていかれる (実測で
`-j2` が 0.34 s、まとめると 0.23 s = `-j1` と同じ)。履歴が無ければ割る —
遅いスイートを速くする形はそちらで、1 回走れば答えが分かる。
...
| **P4** | D2 の所要時間記録 + longest-first | 意図的に偏らせたスイートで wall が最長ジョブ + ε になる (0.8 s のテスト 1 本 + 0.2 s × 8 を `-j4` で: 1.353 → **0.935 s**) | ✅ |
...
- **確保カウンタのリセットは既にあった。** D3 が心配していた
  ...
  元から起きない
- **`-j2` が小さいスイートで損をしていたのは P4 が直した。** 合成
  9 ファイルの `-j2` は 0.34 s (`-j1` は 0.22 s) だった — plan と実行の
  両方でファイルごとの front end を払い直すのに並列度が足りない。
  履歴が「このファイルのテストは front end より安い」と言えるように
  なったので、そういうファイルは**まとめて 1 ジョブ**にする (0.23 s)。
  **「並列度を落とす」のではなく「ジョブを粗くする」のが正解だった** —
  ワーカーを減らすと plan の並列まで道連れになる
- **偏りのあるスイートでは順序が全部を決める。** 0.8 s のテスト 1 本と
  0.2 s のテスト 8 本を `-j4` で走らせると、plan 順 (遅いものが最後) で
  1.353 s、longest-first で **0.935 s** — 後者は「最長ジョブ + ε」で、
  理論値どおり。`-j2` では 1.892 → 1.545 s

## 7. 検証

`toy/tests/package_layout.rs` に足す (4・5 以外は landing 済み):
...
テストが落ちる。**これが並列化の唯一の安全弁**である。P4 が入ってからは
**履歴あり / なしの一致**も同じ理由で要る (スケジュールは実行順を
変えるが、報告は plan 順に組み直す)。
