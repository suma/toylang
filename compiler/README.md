...
- **`print(x)` / `println(x)`**: `i64` / `u64` / `f64` / `bool` / 文字列リテラル / **struct binding / tuple binding / enum binding** を受け取る。compound 値は interpreter と同じ表記 (`Point { x: 3, y: 4 }`、`(3, 4)`、1-tuple は `(x,)`、`Color::Red`、`Shape::Circle(5)`、`Shape::Rect(3, 7)`) に展開される。struct のフィールドはアルファベット順にソート、enum はランタイムで tag を見て該当 variant の表示パスを brif chain で選ぶ（`n - 1` 個の比較、最後の variant は無条件 fallthrough）。ネストした struct も再帰的に整形。実体は `compiler/runtime/toylang_rt.c` の `toy_print_*` / `toy_println_*` ヘルパー経由で stdout に出力（driver が `cc` で同時にコンパイル＋リンク）。**制約**: struct / tuple / enum は識別子（`val` / `var` 由来の binding、または関数引数）のみ。struct/tuple リテラル直接や struct/tuple-returning call の結果、enum リテラル直接（`Enum::Variant`）を print に渡すことはできず、いったん `val` で受ける必要がある
