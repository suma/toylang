...
| match lowering + scrutinee 制約 | `compiler_lower/src/match_lowering.rs`。compound scrutinee (struct / tuple) は `classify_match_scrutinee` が `MatchScrutinee::Struct` / `Tuple` を返し、照合は `dispatch_struct_pattern` / `dispatch_tuple_pattern` / `dispatch_field_shape_pattern`。**arm body の型推論にも束縛が要る** (`bind_*_for_inference`) — 無いと result local が作られない。enum payload の中の struct / tuple パターン (`Some((a, b))`) は `check_payload_sub_pattern` が payload の locals を同じ walk に渡す (ENUM-TUPLE-SUBPATTERN-AOT) |
...
| `Ptr<T>` / `Span<T>` (POINTER P3/P4) | **stdlib のみ** (生 builtin を触る method は `unsafe fn`、P6) — `core/std/ptr.t` / `core/std/span.t` (`Box<T>` と同じ手口)。**ただし呼び出しにしない intrinsic がある** — `Ptr` / `SoaPtr` の `get` / `set` / `borrow` / `load16` / `store16` は `compiler_lower/src/method_call.rs::lower_ptr_access_intrinsic` と tree-walker の `evaluation/call.rs::ptr_access_intrinsic` (MEMORY-ACCESS M5)、`Span` の `copy_from` / `move_from` / `bytes_eq` / `fill` と `val` 束縛の `from_parts` は `compiler_lower/src/span_intrinsic.rs` と `evaluation/call.rs::span_range_intrinsic` (SPAN-RANGE-INTRINSIC)。対象は `std.ptr` / `std.span` で書かれたメソッドだけ (`method_is_from_std_module`)。bracket sugar (`p[i]` / `s[i] = v`) の compiled レーン dispatch だけは `array_access.rs::lower_slice_access` / `lower_slice_assign` (struct binding を `__getitem__` / `__setitem__` 呼び出しに委譲、checker 側の arity / generic 戻り型は `type_checker/struct_literal.rs::check_struct_getitem_access` / `check_struct_setitem_access`)。tree-walker の struct 経由 bracket は `evaluation/slice.rs` |
...
| ヒープ検査 (HEAP-CHECK、`--heap-check=report\|poison\|reuse`) | **状態と判定は 2 か所** — interpreter: `interpreter/src/heap.rs` (`HeapCheckState` / `heap_check_access` が解放済み・末尾越え、`heap_check_freed_again` が二重 free、`quarantine_block` + `free_lists` が reuse、`poison_range`)、compiled: `compiler/runtime/toylang_rt/src/lib.rs` の `hc_*` と `toy_heap_check` / `toy_heap_check_free` / `toy_heap_poison` / `toy_heap_check_start_mode`。**計装**は `compiler_lower/src/lib.rs::emit_heap_checks` (`emit()` の 1 か所で `HeapCheck` / `HeapCheckFree` を前置、`Module.heap_check` のときだけ)、cranelift は `compiler/src/codegen/lower_inst.rs` (枠の前後半は `frame_strings`)、IR VM は `compiler_vm/src/dispatch.rs` → `host.heap_probe*`。IR VM が計装されるのは `interpreter/src/ir_vm/lift.rs` が poison を見たとき。CLI: `interpreter/src/main.rs` / `compiler/src/main.rs` / `compiler/src/all_backends.rs::HeapCheckRun` / `toy/src/main.rs::check_heap_flags`。常時検査は `compiler/tests/example_consistency.rs` の poison スイープ。設計は [`HEAP_CHECK.md`](HEAP_CHECK.md) |
