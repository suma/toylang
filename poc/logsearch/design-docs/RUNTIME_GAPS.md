...
| **容器から取り出したハンドルを束縛すると閉じる** | 診断は出ない。2 回目の `write` が `unknown error` (EBADF) になるだけ | `val s: TcpStream = conns.get(0u64)` は**要素の別名**に drop glue が付くので、その束縛が死ぬと fd が閉じる (`Vec<File>` も同じ)。`Vec<TcpStream>` 自体は作れる。回避は「取り出して束縛しない」か、番号だけの表にして `TcpStream::from_fd` で開き直すこと (**その API が無い** — HTTP サーバの同時接続が 1 本なのはこれが理由)。CLAUDE.md の「compound を返す method から束縛したローカルはコンテナのバッファを解放する」と同じ根で、stdlib 側は `__builtin_ptr_read::<T>` で回避している。**2026-09-18 に 4 レーンで再現を確認** |
