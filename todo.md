...
### 2026-05-08
- **STRING-API-PHASE-1 完了** (`e9b2ee7`) — `core/std/str.t` の `Length` / `AsPtr` extension trait を `Vec<u8>` (= `String`) にも `impl` し、`.len()` / `.as_ptr()` が `str` / `String` 共通の call shape で呼べるように。`core/std/str_ops.t` を新規追加 (将来の `Concat` / `Contains` / `Substring` / `Trim` / `CaseConvert` trait の置き場、現状は header のみ)。新規 `interpreter/tests/string_stdlib_tests.rs` に Phase 1 テスト 4 件 + Phase 0 と合わせて計 14 件、`compiler/tests/consistency.rs` に `string_len_via_length_trait_round_trip` / `string_as_ptr_via_trait_round_trip` の 3-way 2 件。1338 -> **1344 tests pass**。
- **STRING-API-CHAR-U32 完了** (`a0cae0d`) — `core/std/char.t` を `type char = u8` から `type char = u32` (Unicode codepoint) に格上げ。char literal は lexer 段階で既に `Kind::UInt32` として lex されていたが alias が取り残されていた整合性を回復。`Vec<u8>::push_char(c: char)` を **UTF-8 encoding 対応** (RFC 3629、1〜4 bytes)、surrogate (U+D800..U+DFFF) と U+110000 以上は `assert(...)` で panic。副次 fix: `interpreter/src/module_integration.rs::remap_expression` に narrow int (`Int8` / `Int16` / `Int32` / `UInt8` / `UInt16` / `UInt32`) と `Float64` の arm を追加 (NUM-W landing 以来 latent に欠落していた、stdlib body に narrow-int リテラルが入って初めて発火)。新規 `interpreter/tests/string_stdlib_tests.rs` に push_char の UTF-8 ladder テスト 10 件 (ASCII / 'é' / 'あ' / '😀' / surrogate / out-of-range / mixed)、`compiler/tests/consistency.rs` に 3-way 2 件 (`push_char_two_byte_utf8_round_trip` / `push_char_three_byte_utf8_round_trip`)。caller migration: `s.push_char(33u8)` → `33u32` (u8 → u32 の暗黙 widening は無いため明示)。1326 -> **1338 tests pass**。残: STRING-API-PHASE-2 / 3 (substring / trim / case / split / Concat / Contains / ToString) は AOT lower の `compound-returning instance method` + `trait method with struct argument` 制約のため deferred (下記未実装参照)。
...
STRING-API-PHASE-2-3. **String API 拡充の Phase 2 / 3** — Phase 0 (char u32) と Phase 1 (Length / AsPtr) は landed (2026-05-08)。残: (Phase 2) `substring(start, end)` / `trim()` / `to_upper()` / `to_lower()` / `split(sep)` を `Vec<u8>` に impl。(Phase 3) `s.to_string() -> String` の str → String 変換 trait。**前提となる AOT 修正**: (a) **trait method の struct (or `&struct`) 引数 lower** が "method argument produced no value" で fail (Concat / Contains / Split に必須)、(b) **instance method の compound (Self) 戻りを expression position / let-rhs で受ける lower** が "compiler MVP cannot use a compound-returning method (...) in expression position" で fail (substring / trim / case / split / to_string すべてに必須)。両方 interpreter / JIT では動作するので、stdlib 実装と test を別 branch で先行できるが、3-way consistency に乗せるには AOT lower 修正が必須。優先度: 中。
...
### テスト状況
- 合計 1344 テスト, 31 skipped（100% 成功率、2026-05-08 時点 — STRING-API-CHAR-U32 / STRING-API-PHASE-1 で 18 件追加。compiler/e2e は 191、consistency は 50+）
