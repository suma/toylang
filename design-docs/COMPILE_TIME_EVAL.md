...
| **C5** | 型の中の値 — 配列長に `const` / `const fn` / 式を許す | ✅ 2026-08-26 |
...
1. ✅ 配列長に `const` 識別子を書ける (実測 5 が解消)。
   `const N: u64 = 3u64` → `val a: [i64; N]`。`const M: u64 = N` の
   連鎖も辿る。パーサが `type_aliases` と同じ流儀で解決する
   (`Parser::const_lengths`、前方参照は不可)。
2. ✅ **計算が必要な長さ** (`[i64; double(2u64)]`、`[i64; N + 1u64]`)。
   解き方は設計時の (a)「`TypeDecl` が未解決の長さを持てるようにして
   driver の pass で解決する」を採用した:
   - `TypeDecl::Array` の size が `ArraySize` enum
     (`Literal(usize)` / `Deferred(ExprRef)`) になった。
     パーサは リテラル / const 識別子 (従来どおり) と、
     **それ以外の式** (呼び出し・算術・括弧) を式として parse して
     `Deferred` で記録する。
   - 型検査は `Deferred` を「サイズ不明」として扱う — 要素型は
     比較するが **count は比較しない** (`is_equivalent` /
     `same_type_modulo_spelling` / `unify_types` を Deferred 対応にした)。
     → 実害: 宣言長とリテラルの要素数が食い違っても検出されない
     (既知の gap、`compiler_lower` は配列長をリテラルから取るので
     実行時の挙動は正しい)。
   - **C6 の fold が長さ式の中の呼び出しを畳む** — 式は expression
     pool にあるので、既存の pass 2 (全引数リテラルの `const fn` 呼び出し)
     がそのまま畳む。**ただし fold の lowering が const fn 本体を
     entry として lower するようにした**: 型注釈の中の呼び出しは
     どの関数 body からも呼ばれないので、demand-driven lowering が
     callee の body を下ろさず VM が実行できなかった (今回実装で
     見つけた結合)。
   - **解決 pass** (`const_eval.rs::resolve_array_lengths`) が fold の
     直後に走り、`compiler_lower::consts::eval_const_expr` — **lowering
     と同じ literal リーダー** — で畳み残り (リテラル + const 参照 +
     算術) を count にして `Literal` に焼き込む。呼び出しが残って
     いたら E0017 (理由つき: `const fn` でない / 引数が定数でない /
     未知の名前)。
   - `[i64; double(N)]` (const 引数) は**不可**のまま — fold の
     pass 2 の「引数はリテラルのみ」規則 (body 内の名前は const を
     隠すローカルでありうるため) を長さ用に緩めていない。エラー文言
     が「arguments are not all constants」と理由を言う。
3. ✅ **算術 (`[i64; N + 1u64]`)** — 設計時は「パーサに小さな評価器を
   置けば通せるが、それは 4 つ目の評価器で、本文書が防ごうとしている
   ものそのもの」と不可にしていた。C6 で評価器が VM に一本化された
   後は「defer して fold + 既存の literal リーダーで解決」が
   **新しい評価器を一切作らずに**通せるので、同じ理由で解消した。
   パーサは defer の対象を式全体に広げただけで、評価は
   `eval_const_expr` (consts.rs → fold.rs の共通テーブル) が行う。

**C6 の後の追記**: 上記 2 は C6 の評価器一本化が前提になった —
  fold が VM を共有しているからこそ「fold に畳ませてから読む」が
  新しい意味論を持ち込まない。
...
**C5 の残りは C6 の後に回すのが安い、と書いたが、両方 C6 の直後に
  landing した** — 上記 C5 の 2・3 を参照 (C6 の fold 機構が
  `Deferred` 長さの解決の前提になった)。
