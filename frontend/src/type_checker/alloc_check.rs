...
pub fn check_never_allocates(
    ...
    // by name. A name can belong to several types (`Vec::new` and
    // `String::new`), and this pass has no receiver type to pick with,
    // so **all** same-named bodies are walked: a name is rejected if
    // any of them allocates. That is the conservative direction — the
    // alternative, guessing one, would let a real allocation through.
    let mut methods: HashMap<DefaultSymbol, Vec<StmtRef>> = HashMap::new();
    for index in 0..program.statement.len() {
        let stmt_ref = StmtRef(index as u32);
        if let Some(Stmt::ImplBlock { methods: impl_methods, .. }) = program.statement.get(&stmt_ref)
        {
            for method in &impl_methods {
                methods.entry(method.name).or_default().push(method.code);
            }
        }
    }
...
struct Walker<'a> {
    ...
    /// Method bodies by name — see the comment where this is built.
    methods: HashMap<DefaultSymbol, Vec<StmtRef>>,
    ...
    clean: HashSet<usize>,
    /// Same idea for method names, keyed by the symbol.
...
impl<'a> Walker<'a> {
    ...
    fn walk_expr(&mut self, expr_ref: &ExprRef, seen: &mut HashSet<usize>) -> Option<Reason> {
        ...
        match expr {
            ...
            Expr::MethodCall(receiver, method, args) => {
                ...
                self.enter(&method, seen)
            }
            Expr::AssociatedFunctionCall(_, function, args) => {
                if let Some(reason) = self.walk_all(&args, seen) {
                    return Some(reason);
                }
                self.enter(&function, seen)
            }
    ...
    fn walk_method(&mut self, name: DefaultSymbol, seen: &mut HashSet<usize>) -> Option<Reason> {
        if self.clean_methods.contains(&name) {
            return None;
        }
        let bodies = self.methods.get(&name).cloned().unwrap_or_default();
    ...
    fn enter(
        ...
        if self.methods.contains_key(callee) {
            return self.walk_method(*callee, seen).map(|reason| reason.prefix(name));
        }
