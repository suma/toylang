...
- [x] **scope-bound auto-cleanup — Arena (Design A)** (完了: 2026-05-04)
      `with allocator = Arena::new() { ... }` の temporary form を 3 backend で
      auto-cleanup wiring。詳細は下の「Phase 5 部分完了 (Arena auto-cleanup)」エントリ。
- [x] **scope-bound auto-cleanup — FixedBuffer** (完了: 2026-05-04)
      `with allocator = FixedBuffer::new(cap) { ... }` でバッファを scope exit 時に
      release。Arena と同形の wiring (runtime `toy_fixed_buffer_drop`、IR
      `AllocFixedBufferDrop`、AOT codegen、`WithScopeCleanup` enum で 2
      variant 共通化、interpreter `FixedBufferAllocator::reset` impl)。
      詳細は下の「Phase 5 部分完了 (FixedBuffer auto-cleanup)」エントリ。
- [x] **stdlib `Drop` trait scaffolding** (完了: 2026-05-04) — `core/std/drop.t` に `pub trait Drop { fn drop(&mut self) }` を定義、`impl Drop for Arena` / `impl Drop for FixedBuffer` を `core/std/allocator.t` に追加。`arena.drop()` / `fb.drop()` の named-binding 呼び出しが trait 経由で dispatch されるようになった。
- [x] **汎用 RAII (interpreter)** (完了: 2026-05-04) — 任意 user struct の `impl Drop for X { fn drop(&mut self) }` が scope exit 時 (linear / Return / Break / Continue) に LIFO 順で auto-call される。詳細は下の「汎用 RAII (interpreter, 2026-05-04)」エントリ。
- [x] **汎用 RAII の AOT 補完** (完了: 2026-05-04) — `Module.drop_trait_structs` + `FunctionLower.drop_scopes` + `register_drop_for_struct_binding` + `emit_drop_scopes_to_depth` を追加、`Expr::Block` / `terminate_return` / `Stmt::Break` / `Stmt::Continue` で wire。`emit_drop_call` は CallWithSelfWriteback で `Drop::drop` method を invoke。stdlib `Arena`/`FixedBuffer` は除外 (syntactic-sniff path で auto-cleanup されるため二重 drop 回避)。JIT は user-defined `impl Drop for X` 検出時に silent fallback で interpreter 経路へ。3-way `assert_consistent` で interpreter / JIT silent fallback / AOT 全 backend pass。1171 → 1173 tests pass。
- [x] **AllocatorBinding wiring (informational tag)** (完了: 2026-05-04) — `InstKind::HeapAlloc` / `HeapRealloc` / `HeapFree` に `binding: AllocatorBinding` フィールド追加、`FunctionLower::classify_active_allocator_binding()` で lower 時に classify (現状は保守的に常に `Ambient` を返す)。codegen は当面 active-stack dispatch (`toy_alloc_current` + `toy_dispatched_*`) を継続 — tag は informational。**残**: `Static` / `Local` への refinement (active stack が compile-time で判明する場合の特殊化) と、それを使った devirt pass (libc malloc 直接呼び出しに fold)。const-prop / inlining が入る前は意味のある perf 改善は出ない。

### 汎用 RAII (interpreter, 2026-05-04)

任意 user struct で `impl Drop for X { fn drop(&mut self) }` を書ける
ようにし、interpreter で binding が scope exit する時に auto-call
される機構を実装。stdlib の Arena/FixedBuffer 特化 (syntactic
sniff) を user 型に generalize。

**`EvaluationContext` 拡張** (`interpreter/src/evaluation/mod.rs`):
- 新フィールド `drop_trait_structs: HashSet<DefaultSymbol>` —
  Drop impl のある struct symbol のセット。
- 新フィールド `drop_scopes: Vec<Vec<DropEntry>>` — per-active-
  scope の auto-drop record リスト。
- 新ヘルパ:
  - `enter_drop_scope` — push 空 Vec。
  - `run_and_pop_drop_scope` — entries を逆順 (LIFO) に
    `invoke_drop` してから pop。
  - `discard_drop_scope` — error path 用、drop なしで pop。
  - `register_drop_if_needed(name, value)` — value が
    `Heap<Object::Struct>` で struct_sym が
    `drop_trait_structs` に含まれていたら `DropEntry` を
    current scope に append。
  - `invoke_drop` — `drop` method を `method_registry` から
    取り出して `call_method` で実行。`&mut self` だが `Rc`
    共有なので writeback 不要。

**`lib.rs` 拡張**: 新ヘルパ `collect_drop_trait_structs(program)`
で全 `Stmt::ImplBlock { trait_name: Some("Drop"), target_type, .. }`
を walk して target_type 集合を返す。`execute_program` で
computed → `eval.drop_trait_structs` に注入。

**`evaluate_block`** (`statement.rs`): block 入る時
`enter_drop_scope`、本体は `evaluate_block_body` (旧本体を切り
出し)、`Ok(_)` 返値で `run_and_pop_drop_scope`、`Err(_)` で
`discard_drop_scope`。これで linear exit / `Return` /
`Break` / `Continue` の全 path で drop が走り、Err (panic 含む)
では skip。

**val / var 宣言** (`handle_val_declaration` /
`handle_var_declaration`): set_val / set_var の直前に
`register_drop_if_needed(name, &value)` を呼ぶ。Rc 共有なので
drop body 内 `self.field = ...` mutation は caller binding に
visible。

**panic 安全性**: `panic("...")` は `Err(_)` 経路なので drop は
skip。process が exit へ向かう状況で drop を走らせると
double-fault のリスクがあるため (Rust の unwind = drop なし
modeに対応)。

**JIT**: 本格的な JIT codegen が user struct method dispatch を
持たないので silent fallback で interpreter 経路に流れる。RAII
機構もそのまま動く。

**AOT**: 別 phase。CallWithSelfWriteback 経路 (compound &mut T
mutation で既に landing) を再利用しつつ、`FunctionLower` に
scope drop tracking を追加し、`Expr::Block` /
`terminate_return` / `Stmt::Break` / `Stmt::Continue` で
適切な drop を emit する必要がある。

**test** (`interpreter/tests/advanced_syntax_tests.rs`):
- `test_drop_trait_auto_called_at_scope_exit` — 2 binding の LIFO
  drop 順を heap-pointer log 経由 (base-10 sequence エンコード)
  で `21` (b → a) を pin
- `test_drop_trait_fires_on_early_return` — `return 7u64` 後の
  drop 順を `43` (b → a) で pin

両方 interpreter only。1169 → **1171 tests pass**。

### Phase 5 完了 (Drop trait scaffolding + AllocatorBinding wiring, 2026-05-04)

Phase 5 の残 2 項目を最小 viable 形で landing。

**stdlib `Drop` trait** (`core/std/drop.t`):
- `pub trait Drop { fn drop(&mut self) }` を新設。RAII 風の API
  contract を documented で残しつつ、現時点では auto-call wiring は
  既存の Arena / FixedBuffer auto-cleanup (syntactic sniff) に
  限定する旨を doc-comment に明記。
- `core/std/allocator.t::impl Arena` から `fn drop(&mut self)` を
  inherent method から外し、`impl Drop for Arena { ... }` の trait
  impl に移動。FixedBuffer も同形 (`__builtin_fixed_buffer_drop`
  経由)。
- `arena.drop()` / `fb.drop()` の named-binding 呼び出しは trait
  経由で dispatch されるが、build-in 経路と semantics 同一なので
  既存テスト / ユーザコードには影響なし。
- 新 builtin `__builtin_fixed_buffer_drop(handle)` を frontend /
  interpreter / JIT eligibility / AOT lower の 4 layer に登録。

**`AllocatorBinding` wiring** (`compiler/src/ir.rs` +
`lower/expr.rs` + `lower/mod.rs` + `codegen.rs`):
- `InstKind::HeapAlloc` / `HeapRealloc` / `HeapFree` に
  `binding: AllocatorBinding` フィールドを追加。
- 新 helper `FunctionLower::classify_active_allocator_binding()` —
  lower 時に呼ぶと現在の `with` scope 構成から binding を返す
  (今は保守的に常に `Ambient`、将来 `Static(0)` / `Local(local)` /
  `Generic(sym)` への refinement の hook ポイント)。
- codegen 側は `binding: _` で無視、active-stack dispatch
  (`toy_alloc_current` + `toy_dispatched_*`) を継続。tag は
  informational。
- Display impl で `; alloc=ambient` のような postfix を出すので
  IR dump で binding を確認できる。

**test** (`compiler/tests/consistency.rs::drop_trait_named_binding_round_trip`):
- `arena.drop()` / `fb.drop()` (named binding) を 3 backend で
  call、各 with スコープ内 alloc が成功することを pin。
- 既存の Arena/FixedBuffer auto-cleanup テスト + 全 1168 既存
  テストが no regression で通る (1168 → 1169 tests pass)。

**残**: 汎用 RAII (temporary auto-drop generalization with lifetime
inference)、Static/Local binding refinement + devirt pass。

### Phase 5 部分完了 (FixedBuffer auto-cleanup, 2026-05-04)

`with allocator = FixedBuffer::new(cap) { body }` の temporary form
についても 3 backend 全部で auto-cleanup wiring が landing。Arena と
完全に対称な構造。

**runtime** (`compiler/runtime/toylang_rt.c`):
- 新 helper `toy_fixed_buffer_drop(handle)` — fixed_buffer slot 専用に
  tracked allocations を free + bookkeeping arrays clear + `used = 0`。
  non-fixed_buffer slot は no-op (`toy_arena_drop` の defensive policy
  と同じ)。

**JIT mirror** (`compiler/src/jit.rs`): 同形 `toy_fixed_buffer_drop`
extern fn + symbol 登録。

**IR** (`compiler/src/ir.rs`): `InstKind::AllocFixedBufferDrop { handle }`
新 variant + Display。

**AOT codegen** (`compiler/src/codegen.rs`): `rt_fixed_buffer_drop`
FuncId + `RuntimeRefs::fixed_buffer_drop` FuncRef + 対応する arm で
`call(fixed_buffer_drop, [h])` emit。

**WithScopeCleanup enum 化** (`compiler/src/lower/mod.rs`): 既存の
`Vec<Option<ValueId>>` を `Vec<WithScopeCleanup>` に refactor、3
variant (`None / ArenaDrop(h) / FixedBufferDrop(h)`)。
`emit_with_scope_cleanup` が enum variant に応じて `AllocArenaDrop`
または `AllocFixedBufferDrop` を `AllocPop` の直後に emit。

**AOT lowering** (`compiler/src/lower/expr.rs::Expr::With`): 既存の
`Arena::new()` 検出を `InlineAlloc { Arena, FixedBuffer(cap_ref) }`
enum 化、`FixedBuffer::new(cap)` のとき `cap_ref` を lower してから
`InstKind::AllocFixedBuffer { capacity }` で handle 取得 → 既存 path と
同じ AllocPush + scope record + linear/early exit cleanup。

**interpreter** (`interpreter/src/heap.rs::FixedBufferAllocator::reset` +
`evaluation/expression.rs::Expr::With`): `FixedBufferAllocator::reset`
を `Allocator` trait override で実装 (tracked addrs 全 free + used=0)。
`Expr::With` の lexical sniff を `inline_arena: bool` から
`inline_temporary: bool` に rename し `Arena::new()` (0 args) と
`FixedBuffer::new(cap)` (1 arg) の両方を検出、scope exit で同じ
`allocator_rc.reset()` 経路 (Arena / FixedBuffer の override が実 free を
実行、それ以外は default no-op)。

**JIT**: silent fallback で interpreter 経路に流れて自動対応。

**test** (`compiler/tests/consistency.rs`):
- `fixed_buffer_temporary_auto_cleanup_round_trip` — 16-byte quota で
  8+8+1 byte の 3 alloc (3 つ目だけ quota 超過 → NULL)、scope 終了後に
  active stack が default に復帰
- `fixed_buffer_temporary_auto_cleanup_early_return_round_trip` — `with`
  body 内 `return` から関数を抜けた後の active stack 復帰

両方 3-way `assert_consistent`。1166 → **1168 tests pass**。

### Phase 5 部分完了 (Arena auto-cleanup, 2026-05-04)

`with allocator = Arena::new() { body }` の temporary form を 3 backend
全部で auto-cleanup する wiring が landing。`val a = Arena::new()` 経由
の named binding は引き続き user 管理 (`a.drop()` 明示) — lexical sniff
は temporary form だけを対象にする。

**AOT (`compiler/src/lower/expr.rs::Expr::With`)**:
- allocator_expr が `Expr::AssociatedFunctionCall("Arena", "new", [])`
  の形を peek して inline-arena 判定
- `__builtin_arena_allocator()` を直接 emit (`InstKind::AllocArena` →
  U64 handle)、`AllocPush(handle)` で active stack に積む
- linear exit: body 後に `AllocPop` + `AllocArenaDrop { handle }` を直接 emit
- 早期 exit (`return` / `break` / `continue`): 既存の
  `emit_with_scope_cleanup` が `with_scope_arena_drops` stack を walk し、
  各 scope の `AllocPop` の直後に対応する `AllocArenaDrop` を emit

**新フィールド** (`compiler/src/lower/mod.rs::FunctionLower`):
- `with_scope_arena_drops: Vec<Option<ValueId>>` — 1 entry / active scope。
  `Some(handle)` = inline arena auto-cleanup 対象、`None` = named binding /
  raw handle / wrapper 経由 (auto-cleanup なし)。`Expr::With` の lowering で
  push/pop、`emit_with_scope_cleanup` がインデックス `depth-1` を参照。

**interpreter (`interpreter/src/evaluation/expression.rs::Expr::With`)**:
- 同形の lexical sniff で `inline_arena: bool` を計算
- `allocator_stack.pop()` の後に `allocator_rc.reset()` を呼ぶ
  (`Allocator` trait の default は no-op、`ArenaAllocator::reset` だけが
  実 free を行う — `__builtin_arena_drop` 経路と同じ semantics)
- body が error で終わった場合も `reset` は走る (panic / 早期 return /
  通常 exit の全 path)

**JIT**: silent fallback で interpreter 経路に流れて自動対応。

**test** (`compiler/tests/consistency.rs`):
- `arena_temporary_auto_cleanup_round_trip` — 2 つの inline arena を
  順次開閉、各 with 終了後に `__builtin_current_allocator() ==
  __builtin_default_allocator()` を pin
- `arena_temporary_auto_cleanup_early_return_round_trip` — `with` body
  内 `return` から関数を抜けた後の active stack が default に戻り、
  arena slot もリークしないことを pin

両方 3-way `assert_consistent` で interpreter / JIT silent fallback /
AOT 全 backend pass。1164 → **1166 tests pass**。
