...
/// `struct addrinfo`.
///
/// **The field order differs from Linux's**: the BSDs put
/// `ai_canonname` before `ai_addr` and Linux puts it after. Reading
/// the wrong one gives a pointer to a name where a sockaddr should be,
/// which is a crash rather than a wrong answer — and it is exactly why
/// this struct is declared per platform instead of once.
#[repr(C)]
struct AddrInfo {
    ai_flags: i32,
    ai_family: i32,
