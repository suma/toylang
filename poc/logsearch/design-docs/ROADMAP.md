...
| **テスト** | 152 件 + プロパティ 3 本 | `toy test poc/logsearch` が 3.3 秒 (AOT、キャッシュ有り)。内訳は下記 |

### 次にやるなら

1. **テスト** — 152 件。内訳は `server` 29 / `http` 23 / `lsz` 14 /
   `catalog` 11 / `search_query` 9 / `ontology_index` 9 / `mount` 8 /
   `main` 7 / `segment_format` 7 / `query` 7 / `ontology_extract` 5 /
...
4. ~~**HTTP サーバ**~~ — 2026-09-11 に landing。`src/http.t` /
   ...
   **`/v1/labels` は 2026-09-20 にラベル辞書から答えるようになった** —
   マウントごとの `meta/labels.dict` を、セグメントを書いたときに足し、
   保持期限で捨てるときに引く。応答の `segments` が 0 になる
   (辞書を持たないマウントは従来の全走査に落ちる)。
   **同時接続は 2026-09-19 に 128 になった** — 接続表を
   番号 (`Vec<i32>`) で持ち、使う番だけ `from_fd` で所有を取り戻す
   形だった。**2026-09-20 に `Vec<Option<TcpStream>>` へ移した** —
   空きは `None`、使うときは `borrow`、閉じるときは `Vec::replace`
   (ELEMENT-BORROW と `Vec::replace` が入ったのでできた)。実測は
   20 並列の `/healthz` が全部 200、`/v1/query` 8 本並列も全部 200
