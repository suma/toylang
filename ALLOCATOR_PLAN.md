...
- [ ] **汎用 Drop trait + RAII** — 任意の struct で `impl Drop for ... { fn drop(&mut self) }`
      を書けて、scope exit 時 / temporary lifetime 終了時に自動呼び出し。
      lifetime / 例外時の drop 順序 / panic 安全性などの設計議論が必要。
      Arena auto-cleanup の syntactic sniff で typical case をカバーした
      ので緊急ではない。
