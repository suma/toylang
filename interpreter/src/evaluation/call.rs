...
/// DATA-ORIENTED Phase 1: the two fields the tree-walker's column
/// window carries in place of an address — the array it views and the
/// field it selects. Named with a `__` prefix because a user struct
/// cannot declare a field starting that way, and they are internal to
/// this engine: the compiled lanes' `Column<T>` holds `addr` / `len` /
/// `stride` instead (`core/std/column.t`).
const COLUMN_SOURCE_FIELD: &str = "__column_source";
const COLUMN_FIELD_FIELD: &str = "__column_field";
...
impl EvaluationContext<'_> {
    ...
    fn build_column_view(&mut self, source: RcObject, field: DefaultSymbol) -> Object {
    ...
    /// The `(array, field)` a column window views, if this struct is
    /// one this engine built. A `Column` value that came from
    /// anywhere else (there is no other way to make one today) has no
    /// source fields and falls through to ordinary dispatch.
    fn column_source(
        interner: &DefaultStringInterner,
    ...
    pub(super) fn evaluate_method_call(&mut self, obj: &ExprRef, method: &DefaultSymbol, args: &[ExprRef]) -> Result<EvaluationResult, InterpreterError> {
        ...
        let column_window = match &*obj_borrowed {
            ...
            {
                Self::column_source(self.string_interner, fields)
            }
            _ => None,
        };
