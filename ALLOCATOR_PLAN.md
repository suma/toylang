...
### Phase 2b: 関数呼び出しでの bound 検証（部分完了: 2026-04-19）

**実装済み:**

- `visit_generic_call` で制約解法後に `fun.generic_bounds` を検査
...
- [ ] `struct Foo<A: Allocator>` の bound 対応（現在は struct 宣言時の bound を捨てている）
- [ ] `impl<A: Allocator> ...` の bound 対応（同上）
- [ ] 複数 bound の構文（`<A: Allocator + Clone>` 等）と trait 定義の導入

### Phase 3: コレクション型 + IR 整備

- [ ] `List<T>` の基本実装（push/pop/get）
