...
### 2026-09-11
- **TEST-PARALLEL P0〜P4 — `toy test` が並列に走る** — `-j N` (既定は
  ...
  **P4**: 所要時間を `build/<profile>/.testtimes` に記録して長い
  ジョブから配る (偏ったスイートの `-j4` が 1.353 → 0.935 s =
  最長ジョブ + ε)。同じ履歴が「このファイルは割らずにまとめる」も
  決める (ミリ秒のテストが散っているスイートの `-j2` が 0.34 → 0.23 s)。
...
- **TEST-PARALLEL の残り (P5 / P6)** ★ — **P0〜P4 は 2026-09-11 に
  landing** (完了済み節)。残り 2 つ:
  * **P5 `serial`** — `test "..." serial { }`。共有資源 (同じファイル /
    固定ポート / cwd 相対の一時ファイル) を触るテストの逃げ道。
    今は `-j1` しかない
