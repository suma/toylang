...
- Stdlib types:
  ...
  - `Ptr<T>` (`core/std/ptr.t`, POINTER P3+P5) — **型付きポインタ窓**。`T` は field に現れず `addr: ptr` の背後にだけ居るので backend 特殊扱いゼロ (`Box<T>` と同じ手口)。`alloc(count)` (stride は `__builtin_sizeof::<T>()`、**0 要素でも 1 バイト確保して非 null を保証**) / `get` / `set` / `borrow(i) -> &T` / `p[i]` / `p[i] = v` (`__getitem__` / `__setitem__`) / `offset(count)` / `as_raw()`。**stdlib の `get` / `set` / `borrow` / 添字は全レーンで呼び出しにならない** (intrinsic、compound の `T` も) ので、`Vec` / `String` / `Dict` / `Box` / `Deque` / `Set` はメソッド内で `Ptr { addr: self.data }` を作って読み書きする (MEMORY-ACCESS M5)。列分割版の **`SoaPtr<T>`** (`addr` + `cap`、同じく intrinsic) を `SoaVec` が使う。**window であって owner ではない** — free は呼び出し側 (`__builtin_heap_free(p.as_raw())`)、index は unchecked。**non-null 不変** (P5) — 不在は `Option<Ptr<T>>` で表す (`has_next: bool` 方式が消える、16 バイト・niche 最適化は不可)。不変は構成による規約で compiler 強制は無し (field visibility は未強制)
