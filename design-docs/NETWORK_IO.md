...
**~~compiled レーンで残る穴~~ — 2026-09-01 に両方解消。**
`var s = match conn { Result::Ok(s) => s, ... }` は COMPOUND-BLOCK-RHS
で、`Result<(), E>` は UNIT-TYPE-ARG で通るようになった。どちらも
net が作った制約ではなく、**net を書いて初めて実プログラムで
踏んだ**もの。AOT でコンパイルしたバイナリが実際に TCP 往復する。
