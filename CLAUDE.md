...
- **`trait` 宣言と `impl <Trait> for <Type>`**: 共通インターフェースを定義する仕組み。
  ...
  - **`dyn Trait` Phase 1 / interpreter** (A5-P1、2026-05-18): `&dyn TraitName` 形式の trait object dispatch を interpreter で実装。`fn describe(a: &dyn Animal) -> i64 { a.sound() }` の引数に任意の `impl Animal for X` を満たす struct を渡せる。実装は **`TypeDecl::Dyn(DefaultSymbol)`** 新 variant (trait symbol を持つ)、新 lexer keyword `dyn`、`parse_type_declaration` で `dyn TraitName` を消費。type checker は `is_arg_compatible_dyn_aware` ヘルパで context-aware coercion (`&Struct → &dyn Trait` および bare `T → &dyn Trait` auto-borrow、`struct_implements_trait` 経由で trait conformance を check)、receiver が `Dyn(trait_sym)` の method dispatch を trait signature table 経由で resolve。interpreter runtime arg check (`evaluation/call.rs:656`) は expected が `Dyn` のとき skip (static check が canonical、runtime は erasure ベース)。**backend coverage**: interpreter のみ。AOT は `compiler/src/lower/types.rs::map_type` で `Dyn` が catch-all で `None` を返すので lower 時に clean fail、cranelift JIT は eligibility scalar map で同様に skip して silent fallback。**今後の phase**: P2 (AOT fat pointer + vtable codegen)、P3 (JIT)、P4 (`Box<dyn Trait>` / heap-owned trait object)
  - **未対応（後続 phase）**: trait 継承 (A3)、associated types (A4)、`dyn Trait` の AOT/JIT/Box 対応 (A5-P2/P3/P4)、generic trait body 内での T 参照
