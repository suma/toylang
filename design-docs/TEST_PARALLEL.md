...
| | 内容 | 完了条件 |
|---|---|---|
| **P0** | §3 の X0〜X4 | 並列にしなくても正しい。X0 は今日でも「同名ファイルが 2 つあると片方のバイナリしか残らない」 |
| **P1** | ジョブ化 + `-j` (AOT はファイル単位 / VM はファイル単位) | `-j1` と `-jN` の出力がバイト一致。合成 8 files が 0.42 → 0.2 s 未満 |
| **P2** | D1 の driver index 化 | `panics` 4 本が 1 コンパイル。**AOT が全部の失敗を報告する** |
| **P3** | D3 の per-test ジョブ + `prepare_tests` | `poc/logsearch` VM が 2 s 未満。X2 が消える (`toy test <名前> --backend vm` が 1 本ぶんの時間で終わる) |
| **P4** | D2 の所要時間記録 + longest-first | 意図的に偏らせたスイートで wall が最長ジョブ + ε になる |
| **P5** | `serial` | `.seg` を書くテストが並列で緑のまま |
| **P6** | `--backend all` (TEST_TOOL T4 の残り) との合成 | レーン × テストがジョブになる |
