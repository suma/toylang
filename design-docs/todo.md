...
- **STRUCT-FIELD-INIT residual: tuple field** ★ — tuple 型の struct field は今も **tuple リテラルでしか**初期化できない (`compiler MVP requires tuple-typed struct field ... to be initialised by a tuple literal`)。struct 型 field と違い、tuple を返す call / 既存束縛を受ける経路が無い。`store_struct_value_into_fields` の tuple 版を書けば同じ形で通る。
