...
### bare 呼び出しは **どの body が訊いているか**で決まる (2026-09-21)

「ユーザ定義が勝つ」には**訊き手の条件**が付く。**モジュールの body の
中では、そのモジュール自身の関数が先**である:

- `import` はモジュールの名前をプログラムに見せる仕組みであって、
  その逆ではない。モジュールは何にインポートされるか知らずに書かれて
  いるので、プログラム側の名前がモジュールの body の意味を変えて
  よいはずがない
- 以前は素の名前が**必ず**ユーザ側の表を先に引いた。`core/std/time.t`
  が自分の `pad2_field` を呼ぶとユーザの同名関数に解決し、
  **シグネチャが合えば黙って違う関数を呼んでいた**

事実は関数自身が持つ (`Function::module_path` /
`MethodFunction::module_path` / `TraitMethodSignature::module_path`、
統合が刻む)。trait の **default body** はその trait のモジュールで
解決する — 本文は trait のものだからで、継承した impl の場所では
ない。3 つの解決器 (型検査器 / tree-walker / lowering) が同じ
規則を読む。
...
- ~~パーサの `a::b::c(...)`~~ — 2026-09-21 に解消。パーサは全セグメントを
  ...
  **同日追記**: `call_paths` は統合が写していなかったので、
  この検査は**モジュールの中では効いていなかった** (`zzz::tbl::f()`
  をモジュールに書くと通る)。写すようにした。`const` の修飾子も
  同じ `[E0030]` で検査する (`MODULE-CONST-PATH`) — 名前空間は
  平らなので**選ぶのではなく検査する**
