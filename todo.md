...
### 2026-05-08
- **OP-OVERLOAD-EXTEND Phase 1-4** (`ae4d358` + `bda35dc`) — Phase B / OP-OVERLOAD-ARITH を全 binary + unary operator に拡張: (Phase 1) `+= -= *= /= %=` 複合代入は parser desugar (`a OP= b → a = a OP b`) で自動経路、AOT は `assign.rs` に struct binding compound-assign arm 追加 (CallStruct で既存 leaf locals に上書き)。(Phase 2) 順序比較 `< <= > >=` を `lt`/`le`/`gt`/`ge` method dispatch (Bool 戻り、Phase B Eq pattern を `try_lower_struct_cmp` に generalise)。(Phase 3) ビット演算 `& | ^ << >>` を `bitand`/`bitor`/`bitxor`/`shl`/`shr` method dispatch (Self 戻り、`let_lowering.rs::Binary` arm の `op_method` table 拡張)。(Phase 4) 単項 `-` `~` `!` を `neg`/`bitnot`/`not` method dispatch (新 path、interpreter `evaluate_unary` + AOT `let_lowering.rs::Unary` arm)。論理演算 `&& ||` は意図的に scope 外 (short-circuit semantics)。新規テスト 17 件 (5 compound + 4 ord + 5 bit + 3 unary) + 4 件 consistency。1402 -> **1423 tests pass** (+21)。MVP 制約: chained expression-position (`a + b + c`)、binary struct literal operand (`a & Bits { v: 1 }`) は follow-up。
- **OP-OVERLOAD-ARITH** (`04eeb25`) — Phase B (`==`/`!=` -> `eq`) を `+` / `-` / `*` / `/` / `%` に generalise。同型 struct ペアで `add` / `sub` / `mul` / `div` / `rem` method (各 `(&self, &Self) -> Self`) に dispatch (3 backend)。frontend type checker は `resolve_numeric_types` の前に struct dispatch を short-circuit (Identifier-Identifier shape での "incompatible types" 回避)、interpreter は `evaluate_binary` の `overload_method_name` table 拡張、AOT は `let_lowering.rs` に arithmetic struct arm 追加 (`CallStruct` で fresh struct binding に流す。compound `Self` 戻りのため `Call` ではなく)。MVP: let-rhs context (`val c = a + b`) のみ対応、chain (`a + b + c`) は follow-up。新規テスト `interpreter/tests/operator_overload_tests.rs` 5 件 (各 arith op) + consistency 1 件。1396 -> **1402 tests pass** (+6)。
...
### テスト状況
- 合計 1423 テスト, 31 skipped（100% 成功率、2026-05-08 時点 — STRING-API 全 Phase + AOT-SIZEOF-COMPOUND + AOT-COMPOUND-PTR-RW + TYPE-ALIAS-QUALIFIER + OP-OVERLOAD (Eq + Arith + Phase 1-4 含む全 binary/unary) + STRING-NOMINAL + STR-INTERP-COMPOUND (struct/tuple/nested) で計 97 件追加。compiler/e2e は 202、consistency は 60+）
