...
- **`trait` 宣言と `impl <Trait> for <Type>`**: 共通インターフェースを定義する仕組み。
  ...
  - **未対応（後続 phase）**: trait 継承 (A3)、associated types (A4)、`dyn Trait` の compound return / multi-return (A5-P2-MVP-D)、JIT 対応 (A5-P3)、`Box<dyn Trait>` (A5-P4)、generic trait body 内での T 参照
