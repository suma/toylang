...
- [x] **stdlib `Drop` trait scaffolding** (完了: 2026-05-04) — `core/std/drop.t` に `pub trait Drop { fn drop(&mut self) }` を定義、`impl Drop for Arena` / `impl Drop for FixedBuffer` を `core/std/allocator.t` に追加。`arena.drop()` / `fb.drop()` の named-binding 呼び出しが trait 経由で dispatch されるようになった。**残**: 任意 user struct の Drop impl を temporary lifetime / scope exit で **自動呼び出し** する RAII 機構。現状は temporary form の `with allocator = Arena::new() { ... }` / `with allocator = FixedBuffer::new(cap) { ... }` の 2 wrapper だけ syntactic sniff で auto-cleanup する。汎用化には lifetime / 例外時 drop 順序 / panic 安全性などの設計議論が必要 — Arena/FixedBuffer 特化が typical case をカバーするので緊急ではない。
