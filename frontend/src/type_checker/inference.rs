...
pub struct TypeInferenceState {
    ...
    /// NUMBER-HINT: the `Expr::Number` nodes the function currently
    /// being checked actually reached. `finalize_number_types` walks
    /// the whole pool, which meant the *first* function checked froze
    /// every later function's unsuffixed literals to the default type
    /// before those functions were ever looked at — so a parameter or
    /// return type that could have claimed them never got the chance.
    /// Finalization consumes this set and clears it per function; the
    /// whole-pool index remains the safety net for nodes no function
    /// body reached.
    pub visited_numbers: Vec<ExprRef>,
}
...
impl TypeInferenceState {
    ...
    fn unify_types(
        ...
        match (left, right) {
            ...
            (TypeDecl::Array(left_elem, left_size, _), TypeDecl::Array(right_elem, right_size, _)) => {
