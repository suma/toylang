...
183. **コンパイラの作成（MVP + IR + panic/assert + print/println + struct + cast/f64 + struct boundary + tuple + const + DbC + --release + nested fields + tuple boundary + 3 経路一致テスト 対応・段階的進行中）** — toylang のソースを実行可能バイナリにコンパイルする独立コンポーネントを新設する。
   ...
   **現在の制約（2026-05-01 時点、live state）** — 詳細は `compiler/README.md`。以下の機能は未対応で、検出時は明確なエラーで reject される:
   ...
   - **enum / match (Phase A1)**: 非ジェネリック enum + unit / tuple variant + `match` (EnumVariant / Wildcard / Name サブパターン) をサポート。payload 型は `i64` / `u64` / `bool` のみ。**制約**: ジェネリック enum、ネストパターン (`Some(Some(x))`)、リテラルパターン、guard、match scrutinee に式（identifier binding に限定）、enum を関数引数 / 戻り値 / `print` に使う、enum 全体の再代入、enum 構築を `if` 等の式戻り値に使う、`f64` payload — すべて未対応
