...
pub fn scan(roots: &[PathBuf]) -> Vec<Collision> {
    ...
    for root in roots {
        ...
        for file in files {
            ...
            for name in top_level_fn_names(&source) {
...
/// Top-level `fn` / `pub fn` names in `source`.
///
...
#[cfg(test)]
mod tests {
    use super::top_level_fn_names;

    #[test]
    fn only_column_zero_functions_count() {
        let src = "\
pub fn alpha(x: u64) -> u64 { x }
fn beta() -> u64 { 0u64 }
unsafe fn gamma() -> u64 { 0u64 }
impl Thing {
    fn delta(&self) -> u64 { 0u64 }
}
";
        let names = top_level_fn_names(src);
