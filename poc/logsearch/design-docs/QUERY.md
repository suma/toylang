...
| 種類 | 例 | 解決の仕方 |
|---|---|---|
| **索引の効くフィールド** | `status=404` `ip=10.0.0.1` `path=/x` `method=GET` `vhost=v` `ua=…` `proto=HTTP/1.1` `host=h` `tag=CRON` と、**行頭のラベル** (`app=api` `level=error`) | 語彙索引の postings。**語が無いセグメントは展開しない** ([`ONTOLOGY.md`](ONTOLOGY.md) §4)。`host` は予約ラベルなので、syslog のヘッダと `host=` ラベルは**同じ語** ([`DATA_MODEL.md`](DATA_MODEL.md) §2) |
...
| **それ以外すべて** | `timeout` `SRC=10.0.0.1` `Host~x` | **本文の部分一致** (AND)。キーがラベルの形 (`[a-z0-9_]{1,32}`) でなければ文字列として探す — 打った人の意図がそれだから。`~` も同じ |
...
**5.d は 2026-09-11 に入った。** 索引が残したレコードがアリーナの
どこに座るかはレコード表が言うので、そこを覆うフレームだけ展開する。
飛ばしたフレームは**展開せず長さだけ進める**ので、オフセットは表が
言うとおりに保たれる (古いバイトはそこに残るが、読む記録が居ない)。

効くのは**選択率ではなくクラスタ性**だった:

| クエリ | 展開したフレーム | 前 | 後 |
|---|---|---:|---:|
| `ip=<一つの住所>` | **8 / 66** | 125 ms | **54 ms** |
| `ua~MJ12bot` | 89 / 132 | 252 ms | 211 ms |
| `path^/wp-` | 102 / 132 | 314 ms | 284 ms |
| `status=404` | 109 / 132 | 402 ms | 390 ms |

1 つの住所は**時間的にまとまる**のでアリーナ上でもまとまり、66 フレーム
中 8 で足りる。ステータスコードはどこにでもあるので、件数が絞れても
フレームは絞れない。**「選択率の低いクエリに効く」は正確ではなく、
「値がまとまっているクエリに効く」**である。
