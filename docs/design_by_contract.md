...
- **自由関数もメソッドも掃かれる**（DBC-CHECK-METHODS）。`impl` 内の契約付き
  メソッドは `StructType::method` の名前で報告され、`self` レシーバは
  **フィールドを再帰的に埋めて生成**される（generic impl は型引数を
  `i64` → `u64` → `f64` → `bool` の順で試し、レシーバも引数も生成できる
  最初の実体化を使う）。反例は `self` 込みで、レシーバのフィールドも
  引数と同じく shrink される
- **生成できる型は `bool` / `i64` / `u64` / `f64` と、それらだけで構成される
  struct**（フィールドを再帰的に埋める）。それ以外（`str`、enum、`ptr`
  フィールドを持つ型など）を引数・レシーバに持つ関数 / メソッドは
  **黙って対象外**になる。`0 contracted function(s) checked` と出たら
  これを疑う
...
| `i < N` / `i <= N` (N は整数リテラル) | `arr[i]` の境界検査（配列長が N 以下のとき、符号なし添字のみ） |
...
契約付きの自由関数もメソッドも無いか、引数・レシーバの型を生成できない
（`str`・enum・`ptr` フィールドを持つ struct など）。
...
- **名前付きタプル返し**（`-> (q: i64, r: i64)`）が無いので、複数の戻り値
  成分に対する事後条件は書きにくい
- 生成できる型は `bool` / `i64` / `u64` / `f64` と、それらだけのフィールドで
  構成される struct（上記）。enum レシーバは未対応
