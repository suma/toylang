...
| **HTTP サーバ** | 一部動く | `serve` が上がり、`/healthz` `/v1/stats` と管理系に答える。同時接続は 1、`/v1/query` と UI は未 |
| **テスト** | **無い** | 下記 |

### 次にやるなら

1. **テスト** — オントロジー (§3 抽出 / §4 索引 / §5 リンク) は
   2026-09-10 に `tests/ontology_*.t` で 8 本ぶん固めた。**残りは
   query / search / lsz / segfile**。`toy test` の道具は揃っている
   (`tests/*.t`、`core/std/testing.t` のアサーション、`panics` テスト、
   `--bless` のゴールデン)。それ以前の回帰はすべて**目視と `grep` と
   の突き合わせ**で見つけていた
...
4. **HTTP サーバ** — 骨格は 2026-09-11 に landing (`src/http.t` /
   `src/server.t` / `serve`)。**残りは `/v1/query` と Web UI**で、
   どちらも同じ 1 つの作り替えを待っている — `query::run` が stdout へ
   直接書くので、出力先を引数に取る形にすれば応答にも UI にも流せる。
   `/v1/ingest` はその次 (書き込み経路が要る)
