...
> **状態: P0〜P3 landing 済み (2026-09-11)。P4 は一度入れて
> 取り下げた** (`.testtimes` の廃止、同日) — 経緯と失った分は §6 の
> 「P4 を取り下げた」に測定つきで残してある。**P5 (`serial`) は
> 2026-09-21 に landing**。残るのは P6 (`--backend all` との合成)。
...
- **rayon を `toy` に足さない。** ジョブが粗くて work-stealing の細かさが
  ...
  と `AtomicUsize` で足りる。
- ~~**長いジョブから先に (P4)**~~ — **取り下げた** (§6)。ジョブ長の
  分散が大きいので順序が speedup をほぼ決めるのは事実だが、**順序を
  決めるには前の run の実測が要り、それはディスクに残す**しかない。
  今はジョブを plan 順に配る。
...
**分割の代償は front end 1 回ぶん × ワーカー数。** 1 ファイルを
per-test に割ると、そのファイルからテストを取ったワーカーの数だけ
front end を払い直す (検査済みプログラムは `Rc` を持つので共有できない)。
`poc/logsearch` のように**テストが重い**スイートではこれが勝ち筋そのもの
(4.83 → 0.80 s)。ミリ秒のテストが 9 ファイルに散っているスイートでは
払い損で、ワーカーが多ければ隠れるが `-j2` では見える (0.33 s、
`-j1` は 0.22 s)。**どちらなのかはテストの所要時間を知らないと言えず、
それを知るのは前の run だけ**なので、`.testtimes` を捨てた今は
無条件に割る。
...
2. **`test "..." serial { }`** — contextual keyword。`panics` と同じ流儀で
   ...
   **最後に 1 本ずつ**走らせる (並列ジョブが終わってから、が一番単純で
   説明もしやすい)。**2026-09-21 に landing。** 実装の要点:
   - ジョブは 1 本のリストのままで、`serial_from` から後ろが直列。
     ジョブ id が 1 つの空間に収まるので、エラースロットもワーカーの
     カーソルも変わらない
   - AOT では `serial` なテストを**自分だけの driver** にする。
     ファイルの他のテストと driver を共有すると、そちらまで直列に
     なってしまう — `serial` は隣のテストについて何も言っていない
   - `--list` が `serial` を出す (テキストも JSON も)。
     「このスイートがなぜ速くならないか」の答えだから
   - `panics` と順不同 (`serial panics "..."` / `panics "..." serial`)
   - **1 本だけの driver はテスト名でバイナリを分ける** (X0 の再来)。
     driver のバイナリはファイル名から作るので、`serial` を持つ
     ファイルは**1 つのパスを 2 つの driver が欲しがる**ことになり、
     片方が書いている最中にもう片方が実行する
     (「the test driver ended before its first test (killed by a
     signal)」)。入れた当日に `poc/logsearch` で踏んだ。フレークなので
     テストは「実行が通ること」ではなく**バイナリが別名であること**を
     見る
...
| **P4** | D2 の所要時間記録 + longest-first | 意図的に偏らせたスイートで wall が最長ジョブ + ε になる | **取り下げ** (下記) |
| **P5** | `serial` | `.seg` を書くテストが並列で緑のまま | ✅ (2026-09-21) |
| **P6** | `--backend all` (TEST_TOOL T4 の残り) との合成 | 両レーンの結果を突き合わせる | ✅ (2026-09-25、レーンは順に — 下記) |

**P6 はレーンを 1 つのジョブ列に混ぜない** (2026-09-25)。最初の実装は
「レーン × テスト」を 1 つのプールに入れたが、`poc/logsearch` で 4 件が
食い違った — 同じテストの AOT 版と VM 版が**同時に**走り、決まった
パスのマウントを取り合った。`serial` でないテストが約束するのは
**他のテスト**からの独立であって、**自分の 2 つめの写し**からの
独立ではない。そこでレーンは順に走らせ、各レーンの中だけ並列にした
(152 件すべて一致、2 回とも)。wall はレーンの和になる。
...
- **確保カウンタのリセットは既にあった。** D3 が心配していた
  ...
  元から起きない
- **`-j2` が小さいスイートで損をする。** 合成 9 ファイルの `-j2` は
  0.33 s で、`-j1` の 0.22 s より遅い — plan と実行の両方でファイルごとの
  front end を払い直すのに並列度が足りない。既定はコア数なのでここには
  落ちない (0.14 s)

### P4 を取り下げた (2026-09-11)

所要時間を `build/<profile>/.testtimes` に記録して長いジョブから配る形を
一度実装し、**同日に取り下げた** (`.testtimes` を持たない判断)。
測ったものは残しておく — 再検討するときの出発点になる。

| 対象 | 履歴あり | 履歴なし (現在) |
|---|---|---|
| 0.8 s × 1 + 0.2 s × 8 を `-j4` | **0.94 s** | 1.30 s |
| 同上 `-j2` | 1.55 s | 1.89 s |
| ミリ秒テスト 9 ファイル `-j2` | 0.23 s | 0.33 s |
| ミリ秒テスト 9 ファイル 既定 | 0.12 s | 0.14 s |
| `poc/logsearch` VM 既定 | 0.80 s | 0.89 s |
| `panics` 4 本 | 0.09 s | 0.09 s |

- **効いたのは「ワーカーよりジョブが多い」ときだけ。** 既定 (コア数) では
  ほとんどのジョブが同時に始まるので順序は関係がなく、差は 0.02〜0.09 s。
  `-j4` のように**絞ったとき**に 1.30 → 0.94 s と効く
- **履歴は 2 つのことに使えた。** 順序 (longest-first) と、
  **ファイルを割るか否か** (D3)。後者は `-j2` の 0.33 → 0.23 s で、
  「並列度を落とす」のではなく「ジョブを粗くする」のが正解だった —
  ワーカーを減らすと plan の並列まで道連れになる
- **代償は状態。** ビルドディレクトリに run をまたぐファイルが 1 つ増え、
  「前回どう走ったか」でスケジュールが変わる。報告は plan 順に組み直す
  ので**出力は変わらない**が、性能の再現には履歴の有無が効く。
  ディスク上の状態を増やさない判断で取り下げた
- **再検討するなら、状態を持たない形がある。** ファイル単位で
  ワークキューを持ち、**自分が既に検査したファイルから優先して取り、
  空いたら他のファイルから盗む** (盗む側が front end を払う)。
  軽いスイートはワーカーがファイルを 1 つずつ抱えて終わり、重いスイートは
  暇なワーカーが盗んで割れる — **履歴なしで D3 の判断が出る**。
  ただし longest-first の取り分 (偏ったスイートの 1.30 → 0.94 s) は
  戻らない。所要時間は測らないと分からない
