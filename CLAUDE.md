...
- **`trait` 宣言と `impl <Trait> for <Type>`**: 共通インターフェースを定義する仕組み。
  ...
  - **未対応（後続 phase）**: デフォルトメソッド、複数 bound（`<T: A + B>`）、trait 継承、`dyn Trait`、associated types
