...
### 2026-05-08
- **TYPE-ALIAS-QUALIFIER 完了** (`ec83086`) — `frontend::alias_resolution::rewrite_program` の expression loop に `Expr::AssociatedFunctionCall(struct_sym, fn_sym, args)` arm を追加。qualifier が non-generic alias なら alias chain を辿って target の base name を求めて symbol substitute。これにより `val s: String = String::from_str("hello")` / `String::new()` のような alias-qualified associated function call が parse + 3 backend dispatch 可能に。type args (`<u8>`) は val/var annotation から既存の `resolve_struct_instance(struct_name, annotation)` path で recover。test 2 件 (`string_from_str_via_alias` / `string_new_via_alias`) + 3-way consistency 1 件 (`string_from_str_via_alias_round_trip`)。1371 -> **1374 tests pass**。
...
### テスト状況
- 合計 1374 テスト, 31 skipped（100% 成功率、2026-05-08 時点 — STRING-API-CHAR-U32 / STRING-API-PHASE-1 / -2 / -4 + AOT-SIZEOF-COMPOUND + TYPE-ALIAS-QUALIFIER で計 48 件追加。compiler/e2e は 191、consistency は 50+）
