...
- **GENERIC-ENUM-HOF-USER** ★ — **user 定義**の generic enum に対する HOF (`impl Box<T> { fn map<U>(self, f: fn (T) -> U) -> Box<U> }`) が lower できない。impl-level の `T` が `Generic` ではなく `Identifier` として compiler に届き active substitution に当たらないため、`Function([Identifier(T)], Generic(U))` のまま parameter lowering に落ちる (パーサが impl method body に generic context を通していないのが元)。stdlib の `Option` / `Result` は 2026-08-10 に 3 backend 対応済み。
