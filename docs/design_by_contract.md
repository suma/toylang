...
> **既知の穴: 文字列補間は同じに数えられない。** 2026-08-21 実測で
> `println("n = {n}")` は interpreter が 24 バイト、AOT が 0 バイト。
> **同じ契約が engine によって通ったり落ちたりする。**
> `__builtin_heap_alloc` を直接呼ぶ分にはどこでも一致するので、
> それが直るまではアロケーション契約を「確保が明示的な関数」に
> 限るのが安全 (`design-docs/todo.md` の `MEM-COUNTER-INTERP-DRIFT`)。

カウンタは**リクエスト単位**で、明示的に確保した分については
3 バックエンドで同じ数字になる。
...
- **アロケーション契約は補間を含む関数で engine 差がある**（上記の既知の穴）
