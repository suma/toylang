...
23. **構造体配列の型推論システム根本的改善** 🔥 **優先度：高**
    - **根本原因**（詳細調査完了）：
      - `visit_array_literal` ⟷ `visit_struct_literal`間の相互再帰による無限ループ
      - 複数配列要素への異なるフィールドアクセスで型推論が循環参照状態に陥る
      - 例：`nested[0u64].inner.value + nested[1u64].count`で発生
    - **現在の問題**：
      - `visit_struct_literal`には再帰深度制限があるが`visit_array_literal`にはない
      - 型推論キャッシュが相互再帰ケースで効果的でない
      - 構造体配列専用の型推論処理が存在しない
    - **解決策の候補**：
      - `visit_array_literal`への再帰深度制限追加
      - 構造体配列専用の型推論パスの実装
      - 型推論キャッシュシステムの改善（相互参照対応）
      - 配列要素型の事前決定による循環回避
    - **テスト状況**：
      - 単純なケース（`arr[0u64].x`、`arr[0u64].inner.value`）は正常動作
      - 複雑な組み合わせのみでスタックオーバーフロー発生
...
- **型推論システムの現在の制限事項**：
  - 構造体配列の複雑なフィールドアクセス（`nested[0u64].inner.value + nested[1u64].count`）でスタックオーバーフロー
  - `visit_array_literal`と`visit_struct_literal`間の相互再帰による無限ループ問題
  - 単純なケースは正常動作するが、複数要素・フィールドの組み合わせで循環参照発生
  - 根本的解決には型推論アルゴリズムの改善が必要（優先度：高）