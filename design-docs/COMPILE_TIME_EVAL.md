...
2. ❌ `[i64; double(2u64)]` は**しない**。設計時に見えていなかった
   順序の制約がある:

   > 配列長は **parse 時に `TypeDecl::Array` に焼き込まれ**、
   > 以降のすべてのパスが数値として読む。一方 CTFE は
   > **型検査済みのプログラムを要求する** (C3: 型が通らないコードを
   > 走らせない)。つまり「長さが要る時点」は「fold が走れる時点」より
   > **前**であり、`trait ConstEvaluator` を frontend に注入しても
   > 順序は変わらない。

   解くには (a) `TypeDecl` が未解決の長さを持てるようにして driver の
   pass で解決する (TypeDecl の形が変わるので影響範囲が広い)、または
   (b) 当該の `const fn` だけを先に型検査して評価する、のどちらか。
   **C6 で評価器を IR VM に一本化した後にやるほうが安い** ((b) の
   「一部だけ型検査して走らせる」が自然に書けるようになる)。
3. 算術 (`[i64; N + 1]`) も同じ理由で不可。パーサに小さな評価器を
   置けば通せるが、それは**4 つ目の評価器**で、本文書が防ごうとして
