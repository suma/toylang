...
| **`match` の腕で受けたハンドルは複製で、元が閉じる** ★★ | 診断は出ない。相手から見ると接続が切れるだけ | `match r { Result::Ok(c) => { ... } }` の `c` は compiled レーンでは**複製**で、元の payload は**スコープの終わりに drop される** — ループの中で `TcpStream::connect` すると、反復のたびに接続が閉じる (サーバ側は最初の read の直後に EOF を見る)。interpreter は別名なので閉じない = **レーンで意味が割れる**。回避は `var x = match r { Ok(c) => c, ... }` の形で**外に移す**か、ハンドルを作らない口を使う (`TcpListener::accept_fd`)。**2026-09-19 に 4 レーンで確認**、サーバの接続表を書くときに踏んだ |
| **容器から取り出したハンドルを束縛すると閉じる** | 診断は出ない。2 回目の `write` が `unknown error` (EBADF) になるだけ | `val s: TcpStream = conns.get(0u64)` は**要素の別名**に drop glue が付くので、その束縛が死ぬと fd が閉じる (`Vec<File>` も同じ)。`Vec<TcpStream>` 自体は作れる。回避は「取り出して束縛しない」か、番号だけの表にして開き直すこと — **`into_fd` / `from_fd` が 2026-09-18 に入った**ので後者が書ける (`NETWORK_IO.md`「接続の表は番号で持つ」)。HTTP サーバの同時接続が 1 本なのはこれが理由で、**表を `Vec<i32>` にすれば `server.t` だけで 128 にできる**。CLAUDE.md の「compound を返す method から束縛したローカルはコンテナのバッファを解放する」と同じ根で、stdlib 側は `__builtin_ptr_read::<T>` で回避している。**2026-09-18 に 4 レーンで再現を確認** |
