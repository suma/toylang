...
- [x] **汎用 RAII (interpreter only)** (完了: 2026-05-04) — 任意 user struct の `impl Drop for X { fn drop(&mut self) }` が scope exit 時 (linear / Return / Break / Continue) に LIFO 順で auto-call される。`EvaluationContext` に `drop_trait_structs: HashSet<DefaultSymbol>` + `drop_scopes: Vec<Vec<DropEntry>>` を追加、`evaluate_block` で push/pop、val/var 宣言で `register_drop_if_needed`。panic / Err は drop skip (process が exit に向かうため)。詳細は下の「汎用 RAII (interpreter, 2026-05-04)」エントリ。**残**: AOT 側の auto-drop wiring (CallWithSelfWriteback emit + scope tracking)、break/continue の scope-inside-loop drop、3-way consistency test。
