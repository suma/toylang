...
- [ ] inline arena auto-drop (`with allocator = Arena::new() { ... }` で exit 時自動 drop)
- [ ] AllocatorBinding::Static / Local の lower 配線 — 現状は Ambient 等価 (toy_alloc_current 経由)。compile-time 定数 allocator では libc 直接呼び出しに最適化できる余地
