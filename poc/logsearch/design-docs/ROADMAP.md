...
| **HTTP サーバ** | 大半動く | `/v1/query` (3 形式) / `/v1/stats` / `/healthz` / 管理系。1000 件 280 KB の応答が部分書き込みを跨いで届く。同時接続は 1、UI と取り込みは未 |
| **テスト** | **無い** | 下記 |
...
4. **HTTP サーバ** — 2026-09-11 に landing。`src/http.t` /
   `src/server.t` / `serve` と `/v1/query` の 3 形式。**残りは
   Web UI と `/v1/ingest`** — UI は `/v1/query?format=json` の上に
   HTML 1 枚を載せるだけ (`src/ui.t`、§3)、取り込みは書き込み経路が
   要るので別の仕事になる。`/v1/streams` と `/v1/labels` は
   カタログのラベル辞書待ち
