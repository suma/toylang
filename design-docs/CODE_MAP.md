...
| const パターン (MATCH-CONST-PATTERN) | const の登録: `interpreter/src/lib.rs` の const 登録ループが `register_const_for_patterns` を呼ぶ。書き換え: `type_checker/pattern_match.rs::rewrite_const_patterns` (`visit_match_impl` の冒頭で腕を literal pattern に置き換え、以降の検査はその形を見る) → `apply_const_pattern_rewrites` が scrutinee を鍵に pool へ書き戻す。値は宣言型で型を固定した literal の**複製** (`typed_const_literal`)。バックエンドは名前を見ない |
