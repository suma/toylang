...
- **再帰型は不可** (`[E0013]`): 間接化なしで自分を含む struct / enum は有限な layout を持てないので型検査で拒否される (`struct Node { next: Node }` / `enum List { Cons(i64, List), Nil }` / **型引数経由の `struct Tree { kids: Vec<Tree> }` も**)。cycle を切るのは `ptr` / 関数型 / `dyn Trait` の位置だけで、`&T` は lowering で消えるので切れない。連結リストは arena + index (`interpreter/example/linked_list_arena.t`) か raw `ptr` で書く。`Box<T>` は未実装 (todo.md BOX-T)
