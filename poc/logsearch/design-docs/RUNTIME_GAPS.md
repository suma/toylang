...
> 借用レシーバまで進んだ時点)。**残っていると書いた項目はこの日に
> 最小プログラムで叩き直している** — 「前はそうだった」を残さないため。
> 消えた項目は §Z へ移した。
...
サーバを立てて外から叩くテストが書けない (プロセスを起こせない)。
**同一プロセス内にクライアントを置く**形 (`compiler/tests/consistency/net.rs`
と同じ流儀) で書くか、AOT で組んだバイナリを Rust 側のテストから叩く。
