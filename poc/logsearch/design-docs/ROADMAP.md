...
| **テスト** | 105 件 + プロパティ 2 本 | `toy test poc/logsearch` が 0.6 秒 (AOT、キャッシュ有り)。内訳は下記 |

### 次にやるなら

1. **テスト** — 105 件。内訳は `server` 25 / `http` 23 / `lsz` 14 /
   `catalog` 11 / `ontology_index` 9 / `mount` 8 / `query` 7 /
   `ontology_extract` 5 / `index_scan` 3。**残りは search / segfile と、クエリ実行そのもの**
