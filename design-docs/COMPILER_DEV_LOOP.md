...
| **D7** | 意味論変更時の `assert_consistent` 必須化 (機械的強制) | 規約のみ |
...
### D7 — 意味論変更時の `assert_consistent` 必須化

**問題 (実測)**: `f` バグは **3 バックエンドが同じ意味論を独立に実装**して
いるために 4 箇所の修正を要した。しかも**型チェッカだけ直した時点で、
型は通るが答えが間違う状態**になった (`Option::map` が 10 でなく 50 を返す)。
これに気づいたのは、たまたま実行するテストを書いたからで、
**何も「他のバックエンドも見ろ」と教えなかった**。

**現状**: `compiler/tests/consistency.rs::assert_consistent` という
一致検証の仕組みは**既にあり、よくできている**。問題は opt-in であること。

**提案**: 規約 (D4 で `CLAUDE.md` に記載済み) に加えて機械的な担保を置く。
最小案は、意味論に関わるディレクトリを触った PR で
`compiler/tests/consistency.rs` の差分が空なら CI で警告する程度でよい。
根本解 (バックエンド間の重複自体を減らす / 共有テーブルから生成する) は
大規模なので別途。
